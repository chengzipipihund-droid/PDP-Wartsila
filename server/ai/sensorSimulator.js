...
class SensorSimulator extends EventEmitter {
  ...
  start () {
    ...
    // Try to read from a real serial sensor (Arduino + DS18B20).
    // If serial is unavailable, we show “--” in the UI and do NOT simulate values.
    this._initSerial()
  ...
  _readSensor () {
    if (!this.serialReady) {
      return { temp: null, timestamp: new Date().toISOString() }
    }

    // When serial is ready, we rely on _setTemp() being called from the parser.
    // Use the last known temperature (may be null if nothing has arrived yet).
    return { temp: this.currentTemp, timestamp: new Date().toISOString() }
  }
