...
function AlarmApp() {
  ...
  // Subscribe to live temperature — go gray if no reading for 5 s
  const tempTimeoutRef = useRef(null)
  useEffect(() => {
    return subscribe('SENSOR_READING', ({ temp }) => {
      setSensorTemp(temp == null ? null : Number(temp))
      clearTimeout(tempTimeoutRef.current)
      if (temp != null) {
        tempTimeoutRef.current = setTimeout(() => setSensorTemp(null), 5000)
      }
    })
  }, [])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', minHeight: '100vh' }}>
      {/* ── Main alarm content ── */}
      <div className="min-h-screen bg-[#F2F2F2]" style={{ flex: 1, minWidth: 0 }}>
        {page === 'database' && (
      ...
      {/* ── Shared sidebar (right side, matches Energy dashboard layout) ── */}
      <Sidebar />
    </div>
  )
}
